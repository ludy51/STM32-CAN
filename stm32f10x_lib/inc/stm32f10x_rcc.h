...
void RCC_ADCCLKConfig(uint32_t RCC_PCLK2);//配置ADC时钟周期
...
void RCC_LSEConfig(uint8_t RCC_LSE);//选择开启外部低速晶振
void RCC_LSICmd(FunctionalState NewState);//选择开启内部低速电路
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource);//使用外部低速时钟为RTC时钟源
void RCC_RTCCLKCmd(FunctionalState NewState);//选择使能RTC时钟
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks);
void RCC_AHBPeriphClockCmd(uint32_t RCC_AHBPeriph, FunctionalState NewState);//AHB
void RCC_APB2PeriphClockCmd(uint32_t RCC_APB2Periph, FunctionalState NewState);//APB2时钟线
void RCC_APB1PeriphClockCmd(uint32_t RCC_APB1Periph, FunctionalState NewState);//APB1时钟线

#ifdef STM32F10X_CL
void RCC_AHBPeriphResetCmd(uint32_t RCC_AHBPeriph, FunctionalState NewState);
#endif /* STM32F10X_CL */ 

void RCC_APB2PeriphResetCmd(uint32_t RCC_APB2Periph, FunctionalState NewState);//复位时钟，若复位IO会成悬浮状态
